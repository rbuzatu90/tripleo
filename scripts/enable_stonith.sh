...
for i in `openstack baremetal node list -c Name -f value | grep -i control`; do
  ...
  echo pcs stonith create ipmi-$i fence_ipmilan pcmk_host_list=$i ipaddr=$ip login=$user passwd=$passwd lanplus=1 retry_on=3 power_timeout=180; echo pcs constraint location ipmi-$i avoids $i
  echo '##############'
done
