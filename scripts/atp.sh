...
openstack aggregate create --zone atp-az atp-hag # Create Availability Zone
openstack aggregate add host atp-hag ne-a2e-an-nfviplus-rk1-com1.tcloud # Add a host to a Availability Zone

openstack flavor create atp-cpu-siblings --ram 16384 --disk 50 --ephemeral 20 --vcpus 4
openstack flavor set atp-cpu-siblings --property hw:cpu_policy=dedicated --property  hw:numa_nodes=1 --property hw:cpu_thread_policy=require --property hw:mem_page_size=1048576 # CPU pinning siblings policy ++ Huge pages
    
openstack flavor create atp-cpu-isolate --ram 16384 --disk 50 --ephemeral 20 --vcpus 4    
openstack flavor set atp-cpu-isolate --property hw:cpu_policy=dedicated --property hw:numa_nodes=1 --property hw:cpu_thread_policy=isolate --property hw:mem_page_size=1048576 # CPU pinning thread siblings + Huge pages

glance image-create --name "centos" --visibility public --disk-format qcow2 --container-format bare --progress --file CentOS-7-x86_64-GenericCloud.qcow2 # openstack glance create centos
